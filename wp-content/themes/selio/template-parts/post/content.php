...
<div id="post-<?php the_ID(); ?>" <?php post_class('blog-single-post ' . esc_attr($class)); ?>>
    ...
    <div class="post_info">
        ...
        <div class="post-content clearfix">
            <p></p>
                <?php
                if ((get_theme_mod('selio_installed') && is_search()) || $pagename == "blog-standard") {
                    $content = get_the_excerpt();
                    $content = strip_shortcodes($content);
                    
                    esc_viewe(wp_strip_all_tags(html_entity_decode(wp_trim_words(htmlentities(wpautop($content)), 50, '...'))));
                } elseif (is_single()) {
                    if (get_theme_mod('selio_installed'))
                        add_filter('the_content', 'selio_strip_shortcode_gallery');
                    the_content();
                } elseif (get_theme_mod('selio_installed') && (is_category() || is_tag() || is_archive())) {
                    $content = get_the_excerpt();
                    $content = strip_shortcodes($content);
                    esc_viewe(wp_strip_all_tags(html_entity_decode(wp_trim_words(htmlentities(wpautop($content)), 50, '...'))));
                } else {
                    ...
                    the_content(sprintf(
                                    /* translators: 1: number of continue */
                                    __('Continue reading<span class="screen-reader-text"> "%s"</span>', 'selio'), get_the_title()
                    ));
