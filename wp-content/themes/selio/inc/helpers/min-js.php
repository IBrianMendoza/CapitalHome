...
class JSqueeze
{
    ...
    protected function extractStrings($f)
    {
        ...
        if(!function_exists('selio_custom_sort')){
            function selio_js_replace_c ($m) {
                return strtr($m[0], ' ', "\x7F");
            }
        }
        
        $cc_on && $code = preg_replace_callback("'//[^\'].*?@#3'", 'selio_js_replace_c', $code);
        ...
        $len = selio_plugin_call::sw_count($strings);
    ...
    protected function extractClosures($code)
    {
        ...
        if ($this->argFreq[-1]) {
            ...
            $i = selio_plugin_call::sw_count($f) - 1;
            while ($i) {
                $c = 1;
                $j = 0;
                $l = strlen($f[$i]);
    ...
    protected function makeVars($closure, &$tree, $key)
    {
        ...
        if ($i = selio_plugin_call::sw_count($v) - 1) {
            $w = array();
            while ($i) {
                $j = $c = 0;
                $l = strlen($v[$i]);
        ...
        if (preg_match_all("#//''\"\"[0-9]+(?:['!]|/')#", $closure, $w)) {
            foreach ($w[0] as $a) {
                ...
                $a = selio_plugin_call::sw_count($v);
                for ($i = 0; $i < $a; ++$i) {
                    $k = $v[$i];
                    if (1 === $i % 2) {
                        if (',' === $k[0] || '{' === $k[0]) {
    ...
    private function rsort($array)
    {
        ...
        if(!function_exists('selio_custom_sort')){
            function selio_custom_sort($a, $b) {
                if ($b[2] > $a[2]) {
                    return 1;
                }
                if ($b[2] < $a[2]) {
        ...
        usort($tuples, 'selio_custom_sort');
