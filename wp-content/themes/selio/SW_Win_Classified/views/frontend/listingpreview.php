...
<?php if(selio_plugin_call::sw_count($images) >=1 && $image_files_exists): ?>
...
<?php $i=0;foreach($images as $image): ?>
        ...
        <div class="property-img">
            <img data-fullsrc="<?php echo esc_url(_show_img($image->filename, '1300x800', true, null)); ?>" src="<?php echo esc_url(_show_img($image->filename, '770x483', true, null)); ?>" alt="<?php echo esc_attr('image','selio');?>">
...
<?php endforeach; ?>
 ...
 <?php $i=0;foreach($images as $image): ?>
        ...
        <div class="col-lg-4 col-md-4 col-sm-4 col-4 thumb-img">
            <div class="property-img">
                <img src="<?php echo esc_url(_show_img($image->filename, '770x483', true, null)); ?>" alt="<?php echo esc_attr('image','selio');?>">
            </div><!--property-img end-->
        </div>
        <?php endif;?>
<?php endforeach; ?>
    ...
    <div class="descp-text">
        ...
        <p class='clearfix'>
            ...
            if(!empty($listing->location_id)){
                ...
                echo '<b>'.esc_html__('Location','selio').':</b> '.esc_html($location);
            }
            ?>
        </p>
    </div><!--descp-text end-->
...
else
{
    ...
    echo '<li><h4>' . esc_html($field_children['field_name']) . ':</h4> <span>' . esc_html($field_val) . '</span></li>';
}
?>
...
<?php if(selio_plugin_call::sw_count($images) >= 1 && $document_files_exists): ?>
<div class="descp-text">
        <h3><?php echo esc_html__('Documents files', 'selio'); ?></h3>
    <?php if($document_files_exists): ?>
        <ul>  
            <?php if(selio_plugin_call::sw_count($images) >= 1): ?>
            <?php $i=0;foreach($images as $image): ?>
            <?php if(in_array($image->filetype, $images_types) === FALSE): ?>
                    <li>
                        <a href="<?php echo esc_url(sw_win_upload_dir().'/files/'.$image->filename); ?>"><?php echo esc_html($image->filename); ?></a>
                    </li>
            <?php endif; ?>
            <?php endforeach; ?>
            <?php endif; ?>
...
if(!selio_plugin_call::sw_settings('hide_map_listingpage')){
    ...
    if(!empty($gmap_lat))
    {
        ...
        {
            echo do_shortcode('[walker custom_popup_x="1" htmlmarker_offset_x="17" htmlmarker_offset_y="-5" inner_html="'.$custom_pin.'" marker_url="' .esc_attr($pin_icon). '" metric="'.esc_attr($metric).'" marker_url="'.esc_attr($pin_icon).'" zoom="'.esc_attr($zoom).'" latitude="'.esc_attr($gmap_lat).'" longitude="'.esc_attr($gmap_long).'"]'.str_replace("'", "\'", _infowindow_content($listing, array('show_details'=>false))).'[/walker]');
        }
        else
        {
    ...
    if(function_exists('sw_win_load_ci_function_calendar'))
    {
        ...
        $calendar = $CI->calendar_m->get_by(array('sw_calendar.listing_id'=>$listing->idlisting), true);
         ...
         if(selio_plugin_call::sw_count($calendar))
         {
            ...
            if(selio_plugin_call::sw_count($rates) > 0)
            {

                echo '<div class="similar-listings-posts widget-rates clearfix" id="reservation_calendar">';
                echo '<h3>'.esc_html__('Reservation rates', 'selio').'</h3>';
                
                echo '<div class="content-box">';
                echo '<table class="table table-striped table-hover table-rates">';
                
                echo '<thead>';
                    echo '<tr>';
                        echo '<th>#</th><th>'.esc_html__('Date from', 'selio').'</th>';
                        echo '<th>'.esc_html__('Date to', 'selio').'</th>';
                        echo '<th>'.esc_html__('Day', 'selio').'</th>';
                        echo '<th>'.esc_html__('Hour', 'selio').'</th>';
                        echo '<th>'.esc_html__('Week', 'selio').'</th>';
                        echo '<th>'.esc_html__('Month', 'selio').'</th>';
                    echo '</tr>';
                echo '</thead>';
                ...
                foreach($rates as $rate)
                {
                    echo '<tr>';
                        echo '<td>';
                        esc_viewe(selio_ch($rate->idrates));
                        echo '</td>';
                        echo '<td>';
                        esc_viewe(selio_ch($rate->date_from));
                        echo '</td>';
                        echo '<td>';
                        esc_viewe( selio_ch($rate->date_to));
                        echo '</td>';
                        echo '<td>';
                        esc_viewe( selio_ch($rate->rate_night));
                        echo '</td>';
                        echo '<td>';
                        esc_viewe( selio_ch($rate->rate_hour));
                        echo '</td>';
                        echo '<td>';
                        esc_viewe( selio_ch($rate->rate_week));
                        echo '</td>';
                        echo '<td>';
                        esc_viewe( selio_ch($rate->rate_month));
                        echo '</td>';
                    echo '</tr>';
                }
                echo '</tbody>';
                ...
                echo '*'.esc_html__('Prices in currency: ', 'selio').esc_html(selio_plugin_call::sw_settings('default_currency'));
                echo '<br />';
            }
            
            ?>
            <?php if( !is_user_logged_in()):?>
                <a href="<?php echo esc_url(get_permalink(selio_plugin_call::sw_settings('register_page'))); ?>" class="btn-default btn-book <?php if (!is_user_logged_in()): ?> login_popup_enabled <?php endif;?>"><?php echo esc_html__('Book now', 'selio'); ?></a>
            <?php else:?>
                <a href="#" class="btn-default btn-book btn-open-book"><?php echo esc_html__('Book now', 'selio'); ?></a>
            <?php endif;?>
