...
class SW_Win_Geomap_Widget extends WP_Widget{
    ...
    {
        $options = array(
            'description' => __('Display geo map', 'sw_win'),
            'name' => __('Geo map', 'sw_win'),
        );
        
        $options['name'] = 'SW '.$options['name'];
        
        parent::__construct('SW_Win_Geomap_Widget', $options['name'], $options);
    }
    ...
    public function widget($args, $instance)
    {
        ...
        if(empty($title))$title=__('Geo map', 'sw_win');
...
//        if(empty($atts['text_criteria']))$atts['text_criteria'] = '';
        ...
        sw_win_load_ci_function('Widgets', 'Geomap', array(&$output, $atts, $instance));
...
add_action('widgets_init', 'register_sw_win_geomap_widget');
function register_sw_win_geomap_widget()
{
    register_widget('SW_Win_Geomap_Widget');
}
