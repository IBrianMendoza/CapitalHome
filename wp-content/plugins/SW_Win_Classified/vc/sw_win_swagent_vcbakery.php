...
add_action( 'vc_before_init', 'Swagent_VC' );
function Swagent_VC() {
    if (class_exists('WPBakeryShortCode')) {
        vc_map( array(
        	'name' => __( 'SW Agent', 'sw_win' ),
        	'base' => 'Swagent',
        //	'icon' => 'icon-wpb-ui-separator',
        	'show_settings_on_create' => true,
        	'category' => __( 'Content', 'sw_win' ),
        //"controls"	=> 'popup_delete',
        	'description' => __( 'Agent details', 'sw_win' ),
        	'params' => array(
        		array(
        			'type' => 'textfield',
        			'heading' => __( 'Agent ID', 'sw_win' ),
        			'param_name' => 'id',
        			'description' => __( 'You can enter agent ID or slug', 'sw_win' )
        		)
        	)
        ) );
        
        class WPBakeryShortCode_Swagent extends WPBakeryShortCode {
...
//                if(empty($atts['radius']))$atts['radius'] = 2000;
                ...
                $output.= do_shortcode('[swagent id="'.$atts['id'].'" ]');
