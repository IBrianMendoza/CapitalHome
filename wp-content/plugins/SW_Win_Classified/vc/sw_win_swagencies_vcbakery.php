...
add_action( 'vc_before_init', 'Swagencies_VC' );
function Swagencies_VC() {
    if (class_exists('WPBakeryShortCode')) {
        vc_map( array(
        	'name' => __( 'SW Agencies', 'sw_win' ),
        	'base' => 'Swagencies',
        //	'icon' => 'icon-wpb-ui-separator',
        	'show_settings_on_create' => true,
        	'category' => __( 'Content', 'sw_win' ),
        //"controls"	=> 'popup_delete',
        	'description' => __( 'Agencies listing', 'sw_win' ),
        	'params' => array(
        		array(
        			'type' => 'textfield',
        		...
        		array(
        			'type' => 'textfield',
        			'heading' => __( 'Num agencies', 'sw_win' ),
        			'param_name' => 'num_listings',
        			'description' => __( 'You can enter num agencies limit', 'sw_win' )
        		),

        	)
        ) );
        
        class WPBakeryShortCode_Swagencies extends WPBakeryShortCode {
...
//                if(empty($atts['radius']))$atts['radius'] = 2000;
                ...
                $output.= do_shortcode('[swagencies text_criteria="'.$atts['text_criteria'].'" show_featured="'.$atts['show_featured'].'" num_listings="'.$atts['num_listings'].'" ]');
