...
class SW_Win_Listingagency_Widget extends WP_Widget{
    ...
    {
        $options = array(
            'description' => __('Display listing agency', 'sw_win'),
            'name' => __('Listing agency', 'sw_win'),
        );
        
        $options['name'] = 'SW '.$options['name'];
        
        parent::__construct('SW_Win_Listingagency_Widget', $options['name'], $options);
    }
    ...
    public function widget($args, $instance)
    {
        ...
        $atts = shortcode_atts(array(
            'text_criteria'=>'',
            'lang_Listingagency'=>__('Agency', 'sw_win'),
        ...
        if(empty($title))$title=__('Agency', 'sw_win');
        
        $output = '';
        //sw_win_show_latest_listings($output, array_merge($instance, $atts));
        
        sw_win_load_ci_function('Widgets', 'listingagency', array(&$output, $atts, $instance));
...
if(function_exists('create_function') && version_compare(PHP_VERSION, '7.0', '<'))
   add_action('widgets_init', create_function('', 'register_widget( "SW_Win_Listingagency_Widget" );'));
else
   add_action( 'widgets_init', function(){register_widget( 'SW_Win_Listingagency_Widget' );});
