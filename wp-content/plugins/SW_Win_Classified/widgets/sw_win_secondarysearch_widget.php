...
class SW_Win_Secondarysearch_Widget extends WP_Widget{
    ...
    {
        $options = array(
            'description' => __('Display secondary search form', 'sw_win'),
            'name' => __('Secondary search', 'sw_win'),
        );
        
        $options['name'] = 'SW '.$options['name'];
        
        parent::__construct('SW_Win_Secondarysearch_Widget', $options['name'], $options);
    }
    ...
    public function widget($args, $instance)
    {
        ...
        $atts = shortcode_atts(array(
            ...
            'lang_Refresh'=>__('Refresh', 'sw_win'),
        ...
        if(empty($title))$title=__('Secondary search', 'sw_win');
...
//        if(empty($atts['text_criteria']))$atts['text_criteria'] = '';
        ...
        sw_win_load_ci_function('Widgets', 'Secondarysearch', array(&$output, $atts, $instance));
...
if(function_exists('create_function') && version_compare(PHP_VERSION, '7.0', '<'))
   add_action('widgets_init', create_function('', 'register_widget( "SW_Win_Secondarysearch_Widget" );'));
else
   add_action( 'widgets_init', function(){register_widget( 'SW_Win_Secondarysearch_Widget' );});
