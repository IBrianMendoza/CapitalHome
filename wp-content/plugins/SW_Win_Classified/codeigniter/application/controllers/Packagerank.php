...
class Packagerank extends My_Controller {
	...
	public function manage()
	{
        ...
        // Load view
		$this->data['subview'] = 'admin/packagerank/manage';
        $this->load->view('admin/_layout_main', $this->data);
	}

	public function addpackagerank($id=NULL)
	{
        // Get parameters
        ...
        {
            $this->data['form_object'] = $this->packagerank_m->get($id);
        }

        $rules = $this->packagerank_m->form_admin;
        ...
        {
            $data = $this->packagerank_m->array_from_post($this->packagerank_m->get_post_from_rules($rules));

            $id = $this->packagerank_m->save($data, $id);
            
            wp_redirect(admin_url("admin.php?page=packagerank_manage&function=addpackagerank&id=$id&updated=true")); exit;
        }
        
        // Load view
		$this->data['subview'] = 'admin/packagerank/addpackagerank';
        $this->load->view('admin/_layout_main', $this->data);
	}
    
    public function rempackagerank($id = NULL, $redirect='1')
	{   
        // Get parameters
        $id = $this->input->get('id');
        
        if(is_numeric($id))
            $this->packagerank_m->delete($id);
	}
    ...
    public function datatable()
    {
        ...
        // configuration
        $columns = array('idpackagerank', 'package_name', 'rank', 'package_days', 'package_price' );
        $controller = 'packagerank';
        ...
        {
            $row->edit = btn_edit(admin_url("admin.php?page=packagerank_manage&function=addpackagerank&id=".$row->{"id$controller"}));
            $row->delete = btn_delete_noconfirm(admin_url("admin.php?page=packagerank_manage&function=rempackagerank&id=".$row->{"id$controller"}));
...
//                    }
	...
	public function invoices()
	{
        ...
        // Load view
		$this->data['subview'] = 'admin/packagerank/invoices';
        $this->load->view('admin/_layout_main', $this->data);
	}
	...
	public function addinvoice($id=NULL)
	{
        ...
        if($this->form_validation->run() == TRUE)
        {
            ...
            wp_redirect(admin_url("admin.php?page=packagerank_invoices&function=addinvoice&id=$id&updated=true")); exit;
        }
        
        // Load view
		$this->data['subview'] = 'admin/packagerank/addinvoice';
        $this->load->view('admin/_layout_main', $this->data);
	}
    ...
    {
        // configuration
        $columns = array('idinvoice', 'invoicenum', 'display_name', 'field_10', 'price', 'is_activated', 'sw_invoice.listing_id', 'sw_invoice.subscription_id', 'user_id' );
        $controller = 'invoice';
        ...
        $recordsTotal = $this->{$controller.'_m'}->total_lang(array('subscription_id IS NULL'=>NULL), sw_current_language_id());

        prepare_search_query_GET($columns, $controller.'_m', array('subscription_name'));
        $recordsFiltered = $this->{$controller.'_m'}->total_lang(array('subscription_id IS NULL'=>NULL), sw_current_language_id());
        
        prepare_search_query_GET($columns, $controller.'_m', array('subscription_name'));
        $data = $this->{$controller.'_m'}->get_pagination_lang($length, $start, sw_current_language_id(), FALSE, array('subscription_id IS NULL'=>NULL));
        ...
        foreach($data as $key=>$row)
        {
            $row->edit = btn_edit(admin_url("admin.php?page=packagerank_invoices&function=addinvoice&id=".$row->{"id$controller"}));
            $row->delete = btn_delete_noconfirm(admin_url("admin.php?page=packagerank_invoices&function=reminvoice&id=".$row->{"id$controller"}));
