...
//            c_redirect('admin/settings/index/');
    ...
    public function generated_all_translations($id=1)
	{
        ...
        $languages_themes = array();
        $languages_loco_themes = array();
        
        if(file_exists(WP_CONTENT_DIR.'/languages/') && file_exists(WP_CONTENT_DIR.'/languages/themes/')){
            $languages_themes = array_diff(scandir(WP_CONTENT_DIR.'/languages/themes/') , array('..', '.'));
        }
        
        if(file_exists(WP_CONTENT_DIR.'/languages/') && file_exists(WP_CONTENT_DIR.'/languages/loco/')&& file_exists(WP_CONTENT_DIR.'/languages/loco/themes/')){
            $languages_loco_themes = array_diff(scandir(WP_CONTENT_DIR.'/languages/loco/themes/') , array('..', '.'));
        }
        ...
        $filename_name = 'theme+purpose.zip';
        
        $theme = wp_get_theme();
        $filename_name = $theme->get( 'Name' );
        if(get_theme_mod('multipurpose'))
            $filename_name .= '-'. str_replace(array('multipurpose', '/'), '', get_theme_mod('multipurpose'));
            
        $filename_name .= '_'.$theme->get( 'Version' ).'.zip';
        $filename_name = strtolower($filename_name);
        
        $upload_dir = wp_upload_dir();
        $filename_zip = $upload_dir['path'].'/'.$filename_name;
        $filename_zip_url = $upload_dir['url'].'/'.$filename_name;
        ...
        foreach ($themes as $theme) {
            if(in_array($theme, $enable_themes)) {
                ...
                /* from wp-content /languages/themes/ */
                $theme_lang_path = WP_CONTENT_DIR.'/languages/themes/';
                $zip->addEmptyDir($zip_folder_theme.'/'.$theme);
                $zip->addEmptyDir($zip_folder_theme.'/'.$theme.'/'.$zip_folder_lang);
                foreach ($languages_themes as $languages_themes_v) {
                    if (strripos($languages_themes_v, $theme) !== FALSE) {
                        $zip->addFile($theme_lang_path.$languages_themes_v, $zip_folder_theme.'/'.$theme.'/'.$zip_folder_lang.'/'.$languages_themes_v);
                    }
                }
                /* end wp-content /languages/themes/ */
                
                /* from wp-content /languages/loco/themes/ */
                $theme_lang_path = WP_CONTENT_DIR.'/languages/loco/themes/';
                $zip->addEmptyDir($zip_folder_theme.'/'.$theme);
                $zip->addEmptyDir($zip_folder_theme.'/'.$theme.'/'.$zip_folder_lang);
                foreach ($languages_loco_themes as $languages_themes_v) {
                    if (strripos($languages_themes_v, $theme) !== FALSE) {
                        $zip->addFile($theme_lang_path.$languages_themes_v, $zip_folder_theme.'/'.$theme.'/'.$zip_folder_lang.'/'.$languages_themes_v);
                    }
                }
                /* end wp-content /languages/loco/themes/ */
            }
        }
        /* end themes featch */
