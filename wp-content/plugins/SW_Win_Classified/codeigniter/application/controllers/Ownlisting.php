...
class Ownlisting extends My_Controller {
	...
	public function addlisting($id=NULL)
	{
        ...
        if($this->form_validation->run() == TRUE)
        {
            ...
            $current_listing_data = $this->listing_m->get($id);
            
            if(/*$is_new &&*/ $current_listing_data->is_activated != 1 && sw_settings('listing_activation_required') == 1 )
            {
    ...
    public function woopayment()
    {
        ...
        if(get_woocommerce_currency() != $this->data['form_object']->currency_code)
        {
            ...
            echo __('Different currency code defined in WooCommerce, please change it to: ', 'sw_win').$this->data['form_object']->currency_code;
            echo '<br />'.__('Your current: ', 'sw_win').get_woocommerce_currency();
            echo '</div></div>';
            exit();
        }
