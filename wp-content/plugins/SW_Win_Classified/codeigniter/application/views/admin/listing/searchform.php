...
<h1><?php echo __('Search forms editor','sw_win'); ?></h1>

<div class="bootstrap-wrapper">
    <div class="panel panel-default">
        <div class="panel-heading">
        <h3 class="panel-title"><?php echo __('Search forms data','sw_win'); ?></h3>
        </div>
  ...
  <div class="row">
    <form action="" class="form-horizontal" method="post">
      ...
      <div class="form-group <?php _has_error('type'); ?> IS-INPUTBOX">
        ...
        <div class="col-sm-10">
          <input name="type" value="<?php echo _fv('form_object', 'type', 'TEXT', 'MAIN'); ?>" type="text" id="input_type" class="form-control" placeholder="<?php echo __('Type','sw_win'); ?>" readonly="" />
        </div>
      </div>
...
<div class="el_drag el_style custom" f_direction="NONE" f_type="WHAT_SEARCH" f_id="NONE" rel="WHAT_SEARCH"><?php echo __('WHAT SEARCH', 'sw_win'); ?></div>
<div class="el_drag el_style custom" f_direction="NONE" f_type="WHERE_SEARCH" f_id="NONE" rel="WHERE_SEARCH"><?php echo __('WHERE SEARCH', 'sw_win'); ?></div>
<div class="el_drag el_style custom" f_direction="NONE" f_type="BREAKLINE" f_id="NONE" rel="BREAKLINE"><?php echo __('BREAKLINE', 'sw_win'); ?></div>
<div class="el_drag el_style custom" f_direction="NONE" f_type="CATEGORY" f_id="NONE" rel="CATEGORY"><?php echo __('CATEGORY', 'sw_win'); ?></div>
<div class="el_drag el_style custom" f_direction="NONE" f_type="LOCATION" f_id="NONE" rel="LOCATION"><?php echo __('LOCATION', 'sw_win'); ?></div>
<div class="el_drag el_style custom" f_direction="NONE" f_type="C_PRICE_RANGE" f_id="NONE" rel="C_PRICE_RANGE"><?php echo __('PRICE RANGE', 'sw_win'); ?></div>

<?php if(function_exists('sw_pluginsLoaded_calendar')): ?>
<div class="el_drag el_style custom" f_direction="NONE" f_type="C_BOOKING" f_id="NONE" rel="C_BOOKING"><?php echo __('BOOKING DATE', 'sw_win'); ?></div>
<?php endif; ?>
...
?>
        ...
        <table>
            <tr>
                <td class="box PRIMARY" colspan="2"><span class=""><?php echo __('SEARCH FORM', 'sw_win'); ?></span>
<?php
$fields_value_json_1 = htmlspecialchars_decode($fields_value_json_1);
...
foreach($obj_widgets->PRIMARY as $key=>$obj)
{
    ...
    if($obj->type == "C_PRICE_RANGE") {
        $title = __('PRICE RANGE', 'sw_win');
    }
...
if(is_object($obj_widgets) && is_object($obj_widgets->SECONDARY))
...
<form id="test-form" class="form-horizontal mfp-hide white-popup-block">
    ...
    <div class="control-group">
        <label class="control-label" for="inputDirection"><?php echo __('Direction', 'sw_win'); ?></label>
        <div class="controls">
        <select name="direction" id="inputDirection">
            <option value="NONE"><?php echo __('NONE', 'sw_win'); ?></option>
            <option value="FROM"><?php echo __('FROM', 'sw_win'); ?></option>
            <option value="TO"><?php echo __('TO', 'sw_win'); ?></option>
        </select> 
        </div>
    </div>
    <div class="control-group">
