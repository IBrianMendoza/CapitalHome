<?php

if ( ! defined( 'ABSPATH' ) ) exit; // Exit if accessed directly

$d_address = array(
                'Croatia, Zagreb, Ilica',
                'Croatia, Zagreb, Tratinska',
                'Croatia, Zagreb, Cvjetna aleja',
                'Croatia, Cestica, Dravska Ulica',
                'Croatia, Split, Put skalica',
                'Croatia, Split, Viska Ulica',
                'Croatia, Varazdin, Cehovska Ulica',
                'Croatia, Karlovac, Bohinjska Ulica',
                'Croatia, Rijeka, Sibenska Ulica',
                'Croatia, Osijek, Drinska Ulica',
                'Croatia, Koprivnica, Vinica',
                'Croatia, Bjelovar, Ulica Nikole Sopa',
                'Croatia, Ivanec, Jezerski',
                'Virovitica, Becka Ulica',
                'Daruvar, Ulica Nikole Tesle',
                'Sisak, Lipa Ulica',
                'Sesvete, Ninska Ulica',
                'Pula, Mutilska Ulica',
                'Petrinja, Gromova Ulica',
                'Zabok, Varazdinska Ulica',
                'Novi Marof, Vrtna Ulica'
                );

$d_titles = array(
                'Black Glass House',
                'Retro House',
                'Ozalj Apartment',
                'Beauty Estate',
                'Retro Old',
                'Small Beauty',
                'Classic Americano',
                'Fear Wood',
                'Round towers',
                'Yellow Wood',
                'Sky Apartment',
                'Lux Villa',
                'Premium Hotel',
                'Spa restaurant',
                'Lux House',
                'Green Land',
                'Eco Farm',
                'Bio Glass',
                'Plastic Evo',
                'Red Willage',
                'Dream Wings'
                );

$d_descriptions = array(
                'It comes with light and a nice balcony view, features free Wi-Fi and air conditioning. Also comes with a computer.',
                'in the peaceful neighborhood of Becka Ulica. Every room in this hotel is air-conditioned and features a flat-screen TV with cable channels.',
                'It has nice lighting, a wonderful balcony view, features free Wi-Fi, comes with air-conditioning, and a computer is provided.',
                'It has a nice balcony view, features free Wi-Fi, comes with air-conditioning, and a computer is provided.'
                );







?>