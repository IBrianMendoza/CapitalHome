 ...
 * @since 1.1.0
 */
class Map_Full_Width extends Widget_Base {
	...
	private $defaults = array(
		...
		array(
			'map_zoom' => '9',
			'enable_defined_location' => 'no',
			'map_height_range' => '',
		),
		array(
			'location_lat' => '40.741895',
			'location_lon' => '-73.989308',
			'location_text' => '212 5th Ave, New York',
			'location_icon' => '',
		)
	);
	...
	public function get_title() {
		return __( 'Map Full Width', 'selio-blocks' );
	}
                        ...
                        $this->start_controls_tabs(
                                'style_'.$i.'_tabs',[
                                        ...
                                        'conditions' => [
                                                   'terms' => [
                                                           [
                                                                   'name' => 'item_1_enable_defined_location',
                                                                   'operator' => '==',
                                                                   'value' => 'yes',
                                                           ]
                                                   ],
					...
					$this->add_control(
						...
						[
							...
							'default' => $val,
                                ...
                                elseif(substr_count($key, '_range') > 0)
				{
					$this->add_control(
						$gen_item,
						[
							...
							'type' => Controls_Manager::SLIDER,
                                ...
                                elseif(substr_count($key, '_int') > 0)
				{
					$this->add_control(
						$gen_item,
						[
							...
							'default' => $val,
                                ...
                                elseif(substr_count($key, 'map_zoom') > 0)
				{
					$this->add_control(
						$gen_item,
						[
	...
	protected function render() {
		...
		*/
		...
		?>
<h3 class="vis-hid"><?php echo esc_html__('Visible Heading','selio-blocks');?></h3>
<section id="map-container" class="fullwidth-home-map" <?php if($settings['item_1_map_height_range']['size'] != ''):?> style="height:<?php echo $settings['item_1_map_height_range']['size'];?><?php echo $settings['item_1_map_height_range']['unit'];?>;" <?php endif; ?>>
	<h3 class="vis-hid"><?php echo esc_html__('Visible Heading','selio-blocks');?></h3>
	<?php if(Plugin::$instance->editor->is_edit_mode()): ?>
	<img src="<?php echo SELIO_THEMEROOT.'/assets/images/map.jpg'; ?>" style="width:100%;" />
	<?php else: ?>
            <?php if($settings['item_1_enable_defined_location'] == 'yes'):?>
        
                <?php
                $gmap_lat = sw_settings('lat');
                $gmap_long = sw_settings('lng');

                $data_location = '';
                if(sw_settings('lat') && sw_settings('lng'))
                    $data_location = sw_settings('lat').', '.sw_settings('lng');
                ?>
                <?php
                    $pin_icon = SELIO_IMAGES . '/markers/house.png';
                    $gmap_lat = sw_settings('lat');
                    $gmap_long = sw_settings('lng');
                
                $pin_icon = 'la la-home';

                if(!empty($settings['item_2_location_icon']))
                    $pin_icon = $settings['item_2_location_icon'];
                
                $custom_pin ="<div class='marker-container'><div class='marker-card'><div class='front face'><i class='".$pin_icon."'></i></div><div class='back face'> <i class='".$pin_icon."'></i></div><div class='marker-arrow'></div></div></div>";
                    
                    
                echo do_shortcode('[swmap latitude="' . $settings['item_2_location_lat'] . '"  htmlmarker_offset_x="20"  zoom="'.$settings['item_1_map_zoom'].'" custom_popup_x="-5" inner_html="'.$custom_pin.'" marker_url="' . $pin_icon . '" longitude="' . $settings['item_2_location_lon'] . '"]' .$settings['item_2_location_text'] . '[/swmap]');
                ?>
            <?php else:?>
