...
/*
Plugin Name: Winter Classified Save Search
Plugin URI: http://codecanyon.net/user/sanljiljan
Description: Addon to Save Search with alerts on classified portal
Author: Sandi Winter
...
define( 'SW_WIN_SAVESEARCH_PLUGIN_PATH', plugin_dir_path( __FILE__ ) );

// [Translation support]

add_action( 'plugins_loaded', 'sw_win_pluginsLoaded_savesearch' );

function sw_win_pluginsLoaded_savesearch() {
	// Setup locale
...
function savesearch_admin_notice__error() {
    global $wpdb;

    if(!function_exists('sw_win_classified_version'))
    {
    	$class = 'notice notice-error';
        $message = __( 'Please install Classified plugin to use this "Save Search" addon', 'sw_win');
        printf( '<div class="%1$s"><p>%2$s</p></div>', $class, $message ); 
    }
    else if(sw_win_classified_version() < 1.4 && sw_classified_installed())
    {
    	$class = 'notice notice-error';
        $message = __( 'Please update Classified plugin to use this "Save Search" addon', 'sw_win');
        printf( '<div class="%1$s"><p>%2$s</p></div>', $class, $message ); 
    }
}

add_action( 'admin_notices', 'savesearch_admin_notice__error' );
