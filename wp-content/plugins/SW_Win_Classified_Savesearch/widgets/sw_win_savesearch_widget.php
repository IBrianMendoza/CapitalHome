...
class SW_Win_Savesearch_Widget extends WP_Widget{
    ...
    {
        $options = array(
            'description' => __('Save Search widget', 'sw_win'),
            'name' => __('Save Search', 'sw_win'),
        );
        
        $options['name'] = 'SW '.$options['name'];
        
        parent::__construct('SW_Win_Savesearch_Widget', $options['name'], $options);
    }
    ...
    public function widget($args, $instance)
    {
        ...
        if(empty($title))$title=__('Save Search', 'sw_win');
        
        $output = '';
        //sw_win_show_latest_listings($output, array_merge($instance, $atts));
        
        sw_win_load_ci_function('Widgets', 'savesearch', array(&$output, $atts, $instance));
...
add_action('widgets_init', 'register_sw_win_Savesearch_widget');
function register_sw_win_Savesearch_widget()
{
    register_widget('SW_Win_Savesearch_Widget');
}
